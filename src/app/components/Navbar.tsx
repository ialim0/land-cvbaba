...
const Navbar: React.FC = () => {
  ...
  const mobileNavItems: MobileNavItem[] = [
    ...
    { href: "/login", label: t("navbar.talkToCVBABA"), Icon: RiLoginBoxFill },
  ];

  return (
    <nav
      className={`fixed left-0 w-full z-50 bg-gradient-to-b from-blue-50 to-white shadow-none border-none ${isPromoBannerVisible ? "top-16" : "top-0"}`}
    >
          ...
          <div className="flex-grow flex justify-center space-x-12">
            {navItems.map((item) => (
              <div key={item.name} className="flex flex-col items-center group">
                <Link
                  href={item.path}
                  className="text-gray-600 transition-colors duration-200 text-lg font-medium"
                >
                  {item.name}
                </Link>
                <div
                  className={`w-1 h-1 bg-gray-800 rounded-full mt-1 transition-opacity duration-200 ${pathname === item.path ? "opacity-100" : "opacity-0 group-hover:opacity-100"
                    }`}
                />
              </div>
            ))}
      ...
      <div
        className={`fixed top-0 right-0 h-full bg-gradient-to-b from-blue-50 to-white text-gray-800 z-50 transform ${isOpen ? "translate-x-0" : "translate-x-full"
      ...
      >
        <div className="flex justify-between items-center p-4">
          ...
          <button
            className="text-gray-800 focus:outline-none hover:opacity-75 transition-opacity"
        ...
        <div className="flex flex-col space-y-6 p-6">
          ...
          {mobileNavItems.map(({ href, label, Icon }) => (
            <div key={label} className="flex flex-col group">
              <Link
                href={href}
                className="text-gray-600 flex items-center space-x-3 text-sm"
                onClick={() => setIsOpen(false)}
              >
              ...
              <div
                className={`w-1 h-1 bg-gray-800 rounded-full mt-1 ml-8 transition-opacity duration-200 ${pathname === href ? "opacity-100" : "opacity-0 group-hover:opacity-100"
                  }`}
              />
            </div>
          ))}
